=== Cosmobit ===
Contributors: DesertThemes
Requires at least: 4.7
Tested up to: 6.4
Requires PHP: 5.6
Stable tag: 1.0.85
License: GPLv3 or later
License URI: https://www.gnu.org/licenses/gpl-3.0.html
Tags: one-column, two-columns ,right-sidebar, flexible-header, custom-background, custom-header, custom-menu, editor-style, featured-images, footer-widgets, post-formats, theme-options, threaded-comments, translation-ready, full-width-template, custom-logo, blog, e-commerce, portfolio


== Description ==

Cosmobit is lightweight, highly extendable and multi-purpose WordPress Theme. It will enable you to create almost any type of website with a beautiful & professional design. Cosmobit supports popular WordPress plugins such as Elementor, WPML, Polylang, Yoast SEO, WooCommerce, Contact Form 7, Jetpack, and much more. Cosmobit Pro demo https://preview.desertthemes.com/pro/cosmobit/


== Installation ==
	
1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Cosmobit in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.


== Copyright ==

Cosmobit WordPress Theme, Copyright (c) 2022, Desert Themes
Cosmobit is distributed under the terms of the GNU GPLs



== Credits ==

* Underscores - (C) 2012-2017 Automattic, Inc. - http://underscores.me/
License: GPLv2 or later (https://www.gnu.org/licenses/gpl-2.0.html) 

* Font Awesome 4.7.0 by @davegandy - http://fontawesome.io - @fontawesome
Licenses - Font: SIL OFL 1.1, CSS: MIT License(http://fontawesome.io/license)
Source: http://fontawesome.io

* Owl Carousel 2.3.4 - by @David Deutsch
Licenses - MIT License(https://github.com/OwlCarousel2/OwlCarousel2/blob/master/LICENSE)
Source: https://github.com/OwlCarousel2

* WOW - by Thomas Grainger
Licenses - MIT License (https://github.com/graingert/WOW/blob/master/LICENSE)
Source: https://wowjs.uk/

* Animate Css by @Daniel Eden - http://daneden.me/animate
Licenses - MIT License(https://github.com/daneden/animate.css/blob/master/LICENSE)
Source: https://github.com/daneden/animate.css

* Customizer Controls 
Licenses - MIT license  (https://github.com/Codeinwp/customizer-controls/blob/master/LICENSE)
Source: https://github.com/Codeinwp/customizer-controls

* WP Bootstrap Navwalker
Licenses - GNU GENERAL PUBLIC LICENSE (https://github.com/wp-bootstrap/wp-bootstrap-navwalker/blob/master/LICENSE.txt)
Source: https://github.com/wp-bootstrap/wp-bootstrap-navwalker

* Sainitization
Licenses - GNU General Public License (https://github.com/WPTRT/code-examples/blob/master/LICENSE)
Source: https://github.com/WPTRT/code-examples


* Screenshot Banner Image - https://pxhere.com/en/photo/115778
License: Creative Commons Zero

* Page Title Image - https://pxhere.com/en/photo/1456381
License: Creative Commons Zero

== Changelog ==

@version 1.0.85
* Style improvements - Product Title

@version 1.0.84
* Style improvements - Product Badge

@version 1.0.83
* Style improvements - Block Previous Post

@version 1.0.82
* Style improvements - Block Next Post

@version 1.0.81
* Style improvements - Block Previous Post

@version 1.0.80
* Tested - By WordPress 6.4.2

@version 1.0.79
* Style improvements - Block Next Post

@version 1.0.78
* Style improvements - Block Read More

@version 1.0.77
* Style improvements - Menu Badge

@version 1.0.76
* Tested - By WordPress 6.4

@version 1.0.75
* Style improvements - Block Archives

@version 1.0.74
* Style improvements - Block Gallery

@version 1.0.73
* Style improvements

@version 1.0.72
* Tested - By WordPress 6.3.2

@version 1.0.71
* Style improvements - Block Product Sale Badge

@version 1.0.70
* Fixed - Header Image Issue

@version 1.0.69
* Style improvements - Block Product Sale Badge

@version 1.0.68
* Style improvements - Block Verse

@version 1.0.67
* Style improvements - Block Details

@version 1.0.66
* Style improvements - Block Details

@version 1.0.65
* Tested - By WordPress 6.3.1

@version 1.0.64
* Style improvements - Block WooCommerce Product

@version 1.0.63
* Style improvements - Block WooCommerce My Account

@version 1.0.62
* Tested - By WordPress 6.3

@version 1.0.61
* Style improvements - Block Post Author

@version 1.0.60
* Updated - Theme Description

@version 1.0.59
* Style improvements - Block Post Author

@version 1.0.58
* Style improvements - Block Post Date

@version 1.0.57
* Style improvements - Block Page List

@version 1.0.56
* Style improvements - Block Author Image  

@version 1.0.55
* Style improvements - Woocommerce Product Price 

@version 1.0.54
* Style improvements - Woocommerce Product Title  

@version 1.0.53
* Style improvements - Woocommerce Product Badge 

@version 1.0.52
* Style improvements - Woocommerce Cart 

@version 1.0.51
* Style improvements - Woocommerce Account Icon

@version 1.0.50
* Tested - By WordPress 6.2.2

@version 1.0.49
* Style improvements - Block Cart Button

@version 1.0.48
* Style improvements - Block Cart Checkout Button

@version 1.0.47
* Style improvements - Block Cart

@version 1.0.46
* Style improvements - Block Mini Cart

@version 1.0.45
* Style improvements - Block Code Background Color

@version 1.0.44
* Style improvements - Block Code Text Color

@version 1.0.43
* Tested - By WordPress 6.2

@version 1.0.42
* Style improvements - Block Calendar Caption

@version 1.0.41
* Style improvements - Block Table

@version 1.0.40
* Style improvements - Block Table

@version 1.0.39
* Fixed Style Issue

@version 1.0.38
* Fixed Style Issue

@version 1.0.37
* Style Updated - Block Post Author & Date

@version 1.0.36
* Style Updated - Search Widget

@version 1.0.35
* Fixed - Block Calendar Head Color Issue

@version 1.0.34
* Fixed - Block Calendar Navigation Color Issue

@version 1.0.33
* Fixed - Block Calendar Caption Color Issue

@version 1.0.32
* Updated - Theme Description

@version 1.0.31
* Fixed - Block Quote Color Issue

@version 1.0.30
* Fixed - Block Text Color Issue

@version 1.0.29
* Fixed - Block Button Style Issue

@version 1.0.28
* Fixed - Information Style Issue

@version 1.0.27
* Updated - Theme Screenshot
* Style Updated - Information Section
* Style Updated - Button

@version 1.0.26
* Style improvements - Block Video Caption

@version 1.0.25
* Style improvements - Mobile Menu

@version 1.0.24
* Style improvements - Block Image Caption

@version 1.0.23
* Style improvements - Block Outline Button

@version 1.0.22
* Child Theme Functionality Updated

@version 1.0.21
* Child Theme Functionality Updated

@version 1.0.20
* Style Added - Blockquote

@version 1.0.19
* Fixed - Review Issue

@version 1.0.18
* Tested - By WordPress 6.1

@version 1.0.17
* Added - Code Block Style

@version 1.0.16
* Added - Slider Navigation for Child Theme

@version 1.0.15
* Child Theme Functionality Updated

@version 1.0.14
* Style Added - Login Logout Button
* Child Theme Functionality Added

@version 1.0.13
* Child Theme Functionality Improved

@version 1.0.12
* Child Theme Functionality Added

@version 1.0.11
* Updated - Search Button Border Radius

@version 1.0.10
* Removed - Slider Bubbles

@version 1.0.9
* Style improvements - Block Outline Button

@version 1.0.8
* Style improvements

@version 1.0.7
* Child Theme Functionality Added

@version 1.0.6
* Changed Footer Copyright Content
* Updated - Animation

@version 1.0.5
* Child Theme Functionality Added

@version 1.0.4
* Style improvements - Footer Search

@version 1.0.3
* Style improvements
* Child Theme Functionality Added

@version 1.0.2
* Style improvements

@version 1.0.1
* Welcome Notice Added

@version 1.0
* Removed some sidebar & typography features
* Style improvements

@version 0.9
* Fixed the theme style issue

@version 0.8
* Style improvements
* Updated theme description
* Bugs Fixes

@version 0.7
* Child Theme Functionality Added

@version 0.6
* Child Theme Functionality Added

@version 0.5
* Child Theme Functionality Added

@version 0.4
* Fixed the theme style issue

@version 0.3
* Fixed the theme style issue

@version 0.2
* Added Theme URI
* Added Author URI
* Changed Theme Screenshot
* Added Upsale Link in Customizer

@version 0.1
* Initial release